...
## Content (paste into Filament > Help Articles)
...
A **product** is whatever you sell — a desktop app, a plugin, a SaaS add-on. Each product belongs to your **team** (multi-tenancy is built in) and acts as the bucket for all related license keys.

In the admin, open **Products → New product** and set:
...
- **Single key** from the Filament admin or via API (`POST /api/license-keys`).
- **Bulk create** from the License Keys page — generate up to **1,000 keys** in one batch. The job runs on the `bulk` queue in the background; you receive an in-app notification when it finishes.
...
Export selected keys from the License Keys page as a CSV with your preferred delimiter, or fetch them via the admin API. From here it is your distribution channel — license email, in-app purchase flow, partner portal.
...
- **API request logs** — every call to `/api/*` is logged (IP, user agent, status, duration) and visible in the admin under **Logs**. Retained for 30 days.
...
- **Background jobs** — bulk operations and outgoing notifications run on Horizon supervisors. You can watch them under `/horizon`.
...
1. Open Filament admin → **Help Articles → New**.
2. Paste the title, slug, and excerpt above.
3. Paste the content (everything under "Content" up to "How to publish") into the content field. Markdown rendering applies.
4. Set **Published at** to now.
5. Save.

The article will appear under **/help** and is searchable via Scout.
